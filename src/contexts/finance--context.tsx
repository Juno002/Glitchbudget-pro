...
export function FinanceProvider({ children }: { children: ReactNode }) {
  ...
  const settings = useMemo(() => {
    ...
    return {
      ...
      // Fuerte coerción a array para evitar errores "not iterable"
  ...
  useEffect(() => {
    async function initializeDB() {
        ...
        if (general) {
             const needsFix = !Array.isArray((general as any).expenseCategories) || !Array.isArray((general as any).incomeCategories);
             if (needsFix) {
                await db.settings.put({
                    ...DEFAULT_SETTINGS,
                    ...general,
                    expenseCategories: Array.isArray((general as any).expenseCategories) ? (general as any).expenseCategories : defaultExpenseCatIds,
                    incomeCategories: Array.isArray((general as any).incomeCategories) ? (general as any).incomeCategories : defaultIncomeCatIds,
                });
                setDataVersion(v => v + 1);
             }
        }
  ...
  const getTotals = useCallback((month: string) => {
    const baseFreq = activeSettings?.baseIncome?.freq ?? 'mensual';
    const baseAmount = activeSettings?.baseIncome?.amount ?? 0;
    ...
    const totalGoalContributionsThisMonth = (goalContributions || [])
      .filter(c => c.date.slice(0, 7) === month)
      .reduce((sum, c) => sum + c.amount, 0);
      
    const available = balance - totalGoalContributionsThisMonth;

    return { totalIncome, totalExpenses, balance, available, baseIncome, planned_total };

  }, [incomes, expenses, budgets, goalContributions, activeSettings.baseIncome, monthlyFromBase]);
  ...
  const value: FinanceContextType = useMemo(() => ({
    ...
    setTheme, setStrictMode, setRolloverStrategy, setBaseIncome,
